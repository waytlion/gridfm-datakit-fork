...
# Add gridfm-datakit to sys.path so it can be imported regardless of cwd
GRIDFM_DATAKIT_ROOT = Path(__file__).resolve().parents[2]
...
OUTPUT_FILE = f"{CASE_NAME.strip()}_3yr_precomputed_load_profiles.csv"
RANDOM_SEED = 42
...
def main() -> None:
    ...
    # Notebook behavior: show and change working directory two levels up.
    t0 = perf_counter()
    print("cwd before:", os.getcwd())
    os.chdir("..")
    os.chdir("..")
    print("cwd after:", os.getcwd())
    print(f"[timing] Working directory setup: {perf_counter() - t0:.3f}s")
